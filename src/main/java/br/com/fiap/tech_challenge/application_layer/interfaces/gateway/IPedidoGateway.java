package br.com.fiap.tech_challenge.application_layer.interfaces.gateway;

import java.util.List;

import br.com.fiap.tech_challenge.domain_layer.business_entities.Pedido;
import br.com.fiap.tech_challenge.interface_layer.gateways.exceptions.MyNotFoundException;

public interface IPedidoGateway {

    public Pedido gravarPedido(Pedido pedido) throws Exception;

    public void atualizarPedido(Pedido pedido) throws MyNotFoundException, Exception;

    public void removerPedido(Pedido pedido) throws MyNotFoundException, Exception;

    public List<Pedido> listarPedidos() throws MyNotFoundException, Exception;

}
